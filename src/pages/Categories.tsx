...
import { ArrowLeft, Plus, X, FolderOpen, Loader2, Trash2, CheckCircle2 } from "lucide-react";
...
export default function Categories({ onBack, initialShowForm = false }: CategoriesProps) {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm("Delete this category? Products using it will keep the category name.")) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col">
      <div className="px-4 py-4 flex items-center gap-3 border-b border-[#F3F4F6] dark:border-gray-800 bg-white dark:bg-gray-900 sticky top-0 z-10">
        ...
        <h1 className="text-lg font-bold text-[#111827] dark:text-white flex-1">Categories</h1>
        {!showForm && (
          <button
        ...
        {showForm && (
          <button
            onClick={() => { setShowForm(false); setShowConfirm(false); setNewName(""); setError(""); }}
        ...
        {showForm ? (
          <div className="space-y-4">
            <p className="text-sm text-[#6B7280] dark:text-gray-400">
              Add categories here. Use them when adding products so billing is easier.
            </p>
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl border ${
                  error ? "border-red-300 dark:border-red-700" : "border-[#E5E7EB] dark:border-gray-700"
          ...
          <>
            {loading ? (
              ...
              >
                ...
                <p className="text-sm text-[#22C55E] dark:text-green-400 font-medium">Add categories before adding products</p>
              </div>
            ) : (
              <ul className="space-y-2">
                {categories.map((cat) => (
                  ...
                  >
                    <span className="font-medium text-[#111827] dark:text-white">{cat.name}</span>
                    <button
                      type="button"
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => !isSubmitting && setShowConfirm(false)}>
          <div
            className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-bold text-[#111827] dark:text-white mb-2">Confirm category</h3>
            <p className="text-sm text-[#6B7280] dark:text-gray-400 mb-4">
              Add category <strong className="text-[#111827] dark:text-white">"{newName.trim()}"</strong>?
            </p>
