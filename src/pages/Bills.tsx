...
import { ArrowLeft, FileText, Search, Calendar, Download, Eye, Loader2, Plus, X } from "lucide-react";
...
type FullInvoice = {
  ...
  paid_amount: number;
  notes?: string | null;
  party?: { id: number; name: string; mobile: string | null; address?: string | null };
...
export default function Bills({ onBack, onCreateInvoice }: BillsProps) {
  ...
  // Build YYYY-MM-DD in local time (avoids timezone bugs with toISOString)
  ...
  const fetchInvoices = async () => {
    try {
      ...
      // Backend expects payment_status (and "partially_paid" for partial)
      if (filterStatus !== "all") {
      ...
      if (filterDate !== "all") {
        ...
        if (filterDate === "today") {
          ...
          // Week: Sunday to today (getDay() 0 = Sunday)
      ...
      if (response.data.success && Array.isArray(response.data.data)) {
        const list = response.data.data.map((inv: Record<string, unknown>) => ({
          ...inv,
          status: inv.payment_status === "partially_paid" ? "partial" : (inv.payment_status ?? inv.status),
  ...
  const handleDownload = async (invoice: Invoice) => {
    ...
    try {
      const res = await api.get(`/invoices/${invoice.id}`);
      if (res.data?.success && res.data?.data) setPdfInvoice(res.data.data);
    } catch (e) {
      console.error("Error fetching invoice for PDF:", e);
      setDownloadId(null);
    }
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      html2canvas(pdfRef.current!, { scale: 2, useCORS: true })
        .then((canvas) => {
  ...
  // Client-side: apply date range so list/totals always match selected filter (in case of timezone/server edge cases)
  const getInvoiceDateStr = (inv: Invoice) => {
    const raw = inv.invoice_date;
    if (!raw) return "";
    const d = new Date(raw);
    if (isNaN(d.getTime())) return raw;
    return toLocalDateString(new Date(d.getFullYear(), d.getMonth(), d.getDate()));
  };
  ...
  const totalAmount = filteredInvoices.reduce((sum, inv) => sum + safeTotal(inv), 0);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col">
      ...
      {(viewLoading || viewInvoice) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => !viewLoading && setViewInvoice(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-[#E5E7EB] dark:border-gray-700">
              <h2 className="text-lg font-bold text-[#111827] dark:text-white">Invoice Details</h2>
              <button onClick={() => setViewInvoice(null)} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
              ...
              {viewLoading ? (
                ...
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <p className="text-sm font-semibold text-[#111827] dark:text-white">{viewInvoice.invoice_number}</p>
                  </div>
                  <div className="flex justify-between">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Date</p>
                    <p className="text-sm text-[#111827] dark:text-white">{formatDate(viewInvoice.invoice_date)}</p>
                  </div>
                  <div className="flex justify-between">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Status</p>
                    <span className={`px-2 py-1 rounded-lg text-xs font-medium ${getStatusColor(viewInvoice.payment_status === "partially_paid" ? "partial" : viewInvoice.payment_status ?? "")}`}>
                      {paymentStatusLabel(viewInvoice.payment_status ?? "")}
                    </span>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Bill To</p>
                    <p className="text-sm text-[#111827] dark:text-white">{viewInvoice.party?.name ?? "—"}</p>
                    {viewInvoice.party?.mobile && <p className="text-sm text-gray-600 dark:text-gray-400">{viewInvoice.party.mobile}</p>}
                    {viewInvoice.party?.address && <p className="text-sm text-gray-600 dark:text-gray-400">{viewInvoice.party.address}</p>}
                  </div>
                  {viewInvoice.items && viewInvoice.items.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">Items</p>
                      <div className="border border-[#E5E7EB] dark:border-gray-700 rounded-lg overflow-hidden">
                        {viewInvoice.items.map((item) => (
                          <div key={item.id} className="flex justify-between py-2 px-3 border-b border-[#E5E7EB] dark:border-gray-700 last:border-0 text-sm">
                            <span className="text-[#111827] dark:text-white">{item.product?.name ?? "Item"}</span>
                            <span className="text-[#111827] dark:text-white">{item.quantity} × {formatAmount(Number(item.unit_price))} = {formatAmount(Number(item.total))}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="pt-2 border-t border-[#E5E7EB] dark:border-gray-700">
                    <div className="flex justify-between text-sm py-1">
                      ...
                      <span className="text-[#111827] dark:text-white">{formatAmount(Number(viewInvoice.subtotal))}</span>
                    </div>
                    {Number(viewInvoice.tax_amount) > 0 && (
                      <div className="flex justify-between text-sm py-1">
                        <span className="text-gray-600 dark:text-gray-400">Tax</span>
                        <span className="text-[#111827] dark:text-white">{formatAmount(Number(viewInvoice.tax_amount))}</span>
                      </div>
                    )}
                    <div className="flex justify-between font-bold text-[#111827] dark:text-white pt-2">
                      <span>Total</span>
                      <span>{formatAmount(Number(viewInvoice.total_amount))}</span>
                    </div>
                    {viewInvoice.paid_amount != null && Number(viewInvoice.paid_amount) > 0 && (
                      <div className="flex justify-between text-sm py-1 text-green-600 dark:text-green-400">
                        <span>Paid</span>
                        <span>{formatAmount(Number(viewInvoice.paid_amount))}</span>
                      </div>
                    )}
                  </div>
                  {viewInvoice.notes && (
                    <p className="text-sm text-gray-600 dark:text-gray-400"><span className="font-medium">Notes:</span> {viewInvoice.notes}</p>
                  )}
                </div>
              ) : null}
            </div>
      ...
      {/* Hidden div for PDF generation - inline styles only (no Tailwind) so html2canvas does not see oklch colors */}
      {pdfInvoice && (
        <div
          ref={pdfRef}
          style={{
        ...
        >
          <div style={{ borderBottom: "2px solid #d1d5db", paddingBottom: 16, marginBottom: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <p style={{ fontSize: 14, color: "#4b5563", margin: 0 }}>{userData.shop_address || ""}</p>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <p style={{ fontSize: 14, margin: 0 }}>Invoice #: {pdfInvoice.invoice_number}</p>
                <p style={{ fontSize: 14, margin: "4px 0 0 0" }}>Date: {formatDate(pdfInvoice.invoice_date)}</p>
              </div>
            </div>
          </div>
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, margin: "0 0 4px 0" }}>Bill To:</h3>
            <p style={{ fontSize: 14, margin: 0 }}>{pdfInvoice.party?.name ?? "—"}</p>
            {pdfInvoice.party?.mobile && <p style={{ fontSize: 14, margin: "4px 0 0 0" }}>Phone: {pdfInvoice.party.mobile}</p>}
            {pdfInvoice.party?.address && <p style={{ fontSize: 14, margin: "4px 0 0 0" }}>{pdfInvoice.party.address}</p>}
          </div>
            ...
            <tbody>
              {(pdfInvoice.items ?? []).map((item) => (
                <tr key={item.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                  ...
                  <td style={{ textAlign: "right", padding: "8px 0" }}>{formatAmount(Number(item.unit_price))}</td>
                  <td style={{ textAlign: "right", padding: "8px 0" }}>{formatAmount(Number(item.total))}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div style={{ marginTop: 24, paddingTop: 16, borderTop: "1px solid #e5e7eb", textAlign: "center", fontSize: 14, color: "#6b7280" }}>Thank you for your business!</div>
        </div>
      )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-blue-200 dark:border-blue-800">
            <CardContent className="p-3">
              ...
              <p className="text-sm font-bold text-blue-900 dark:text-blue-100">{formatAmount(totalAmount)}</p>
            </CardContent>
          </Card>
          <Card className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-green-200 dark:border-green-800">
            <CardContent className="p-3">
              <p className="text-xs text-green-700 dark:text-green-300 mb-1">Paid</p>
              <p className="text-sm font-bold text-green-900 dark:text-green-100">{formatAmount(paidAmount)}</p>
            </CardContent>
          </Card>
          <Card className="bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 border-red-200 dark:border-red-800">
            <CardContent className="p-3">
              <p className="text-xs text-red-700 dark:text-red-300 mb-1">Unpaid</p>
              <p className="text-sm font-bold text-red-900 dark:text-red-100">{formatAmount(unpaidAmount)}</p>
            </CardContent>
          </Card>
        </div>
      </div>
              ...
              >
                <CardContent className="p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                        ...
                        {invoice.party?.mobile && (
                          <span className="ml-2 text-xs">• {invoice.party.mobile}</span>
                        )}
                      </p>
                  ...
                  <div className="flex gap-2 pt-3 border-t border-[#E5E7EB] dark:border-gray-700">
                    ...
                    >
                      {downloadId === invoice.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                      Download
                    </button>
                  </div>
                </CardContent>
